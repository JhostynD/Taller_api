...
Para las migraciones despues de tener npm install @prisma/client y npm install prisma --save-dev instalados, se debe configurar el prisma utulizando npx prisma init
...
1. Flujo de Autenticación (Registro → Login → Uso de Token en /tasks)
...
3. Acceso a rutas protegidas (GET /tasks, POST /tasks, etc.)
...
- Flujo:
    1. Se configura una estrategia que valida el token con `JWT_SECRET`.
    2. Passport extrae el token del header Authorization.
    3. Verifica firma y expiración.
    4. Si el token es válido, Passport inyecta el usuario en `req.user`.
